...
// </copyright>
// <describe> #AssetBundleBuild# </describe>
...
public class AssetBundleBuildEditor : EditorWindow {
	...
	[MenuItem("GF/Tools/AssetBundles Options %#O")]
	...
	[MenuItem("GF/Tools/Build AssetBundles %#T")]
	...
	[MenuItem("GF/Tools/Build AssetBundles Targets %#Y")]
	...
	private static void SavePlatformVersion(List<BuildTarget> targets)
	{
		...
		//保存平台信息
		File.WriteAllText(Path.Combine(_config.BuildPath, _assetPlatformVersionText),json);
		//更新资源版本号 -- 保存配置文件
		_config.Version++;
		SaveConfig();
		//打开文件夹
    ...
    private static void CopyResource(BuildTarget target)
    {
        ...
        {
           Debug.LogWarning("CopyResource: Not found targetPath:"+ targetPath);
           return;
        }
